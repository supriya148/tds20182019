...
Module mod26QConvert

    ' Dim fso As New FileSystemObject
    Dim eTDSPath As String, eFileName As String
    Dim oCoMst As New clsCoMst, TStrm As StreamWriter
    Public CPan As Boolean
    Dim Value18 As Date

    Public Sub Convert26Q(frmid As String, filename As String, revised As String, oldRRRNo As Double, TANApplNo As Double)
        ...
        rstC = FetchDataSet("SELECT * FROM Challan26Q WHERE RetnID=" & rstRetn.Tables(0).Rows(0)("RetnID").ToString()) ', Cnn      'Challan Records
        rstD = FetchDataSet("SELECT DT.*, DM.* FROM Deductee26Q AS DT " &
      ...
      rstRetn.Tables(0).Rows(0)("RetnID").ToString()) ' ' Cnn   'Deductee Records
        rstCSum = FetchDataSet("SELECT sum(TaxAmt) as TotAmt,sum(Surcharge) as TotSc, sum(Ecess) as TotEcess," &
               " Sum(Interest) as TotInt, sum(Others) as TotOth, sum(afees) as Totfees FROM Challan26Q WHERE RetnID=" & rstRetn.Tables(0).Rows(0)("RetnID").ToString()) ', Cnn      'Challan Records
        ...
        '        IIf((rstCSum.Tables(0).Rows(0)("totfees").ToString()), 0, (rstCSum.Tables(0).Rows(0)("totfees").ToString()))
        ...
        BH(33) = oCoMst.PRName26
        BH(34) = oCoMst.PRDesg26
        BH(35) = oCoMst.PR26Add1
        BH(36) = oCoMst.PR26Add2
        BH(37) = oCoMst.PR26Add3
        BH(38) = oCoMst.PR26Add4
        BH(39) = oCoMst.PR26Add5
        BH(40) = oCoMst.PR26StateID
        BH(41) = oCoMst.PR26Pin
        BH(42) = oCoMst.PR26Email
        BH(43) = oCoMst.Comobile  'vbNullString   'Mobile No - not provided in this release
        BH(44) = oCoMst.PR26Std
        BH(45) = oCoMst.PR26Phone
        BH(46) = IIf(oCoMst.IsPR26AddChg = True, "Y", "N")
        ...
        End If
        'BH(54) = IIf(String.IsNullOrEmpty(oCoMst.GovtStateID) = "", "", Format(oCoMst.GovtStateID, "00"))
        ...
        BH(59) = oCoMst.PR26PAN   'Trim(oCoMst.TANRegNo)
        ...
        BH(65) = oCoMst.PR26STDAlt
        BH(66) = oCoMst.PR26PhoneAlt
        BH(67) = oCoMst.PR26EmailAlt
        BH(68) = oCoMst.AIN
        BH(69) = oCoMst.gstin
        BH(70) = vbNullString
        strBH = ""
        ...
        If rstC.Tables(0).Rows.Count - 1 > 0 Then
            'rstC.MoveFirst
        End If
        LNo = 3 : RecNo = 1
        Dim CTotal As Long
        For c = 0 To rstC.Tables(0).Rows.Count - 1
            rstChlnDed = FetchDataSet("SELECT DT.*, DM.* FROM Deductee26Q AS DT " &
      ...
      " AND ChallanID=" & rstC.Tables(0).Rows(c)("ChallanID").ToString())   ', Cnn   'Deductee Records 
            ...
            'Debug.Print(rstC.Tables(0).Rows(c)("ChallanID").ToString())
        ...
        BH(32) = "H" Or BH(32) = "L" Or BH(32) = "N" Then
            ...
            Else
                CD(14) = vbNullString
            End If
            '    CD(14) = IIf(oCoMst.CoStatus = "C" And rstC!IsBookEntry = True, rstC!TranVouNo, vbNullString)
            CD(15) = vbNullString
            '    CD(16) = IIf(IsNull(CD(12)) Or CD(12) = "", vbNullString, Format(rstC!BankBrCode, "0000000"))
            ...
            CD(19) = vbNullString
            CD(20) = vbNullString
            CD(21) = vbNullString   'IIf(rstC!Sec = "194LA", "94L", Right(rstC!Sec, 3))
            ...
            'CD(22) = Format(IIf((rstC.Tables(0).Rows(0)("TaxAmt").ToString()), 0, Val(rstC.Tables(0).Rows(0)("TaxAmt").ToString())), "0.00")
            'CD(23) = Format(IIf((rstC.Tables(0).Rows(0)("Surcharge").ToString()), 0, Val(rstC.Tables(0).Rows(0)("Surcharge").ToString())), "0.00")
            'CD(24) = Format(IIf((rstC.Tables(0).Rows(0)("ECess").ToString()), 0, Val(rstC.Tables(0).Rows(0)("ECess").ToString())), "0.00")
            'CD(25) = Format(IIf((rstC.Tables(0).Rows(0)("Interest").ToString()), 0, Val(rstC.Tables(0).Rows(0)("Interest").ToString())), "0.00")
            ' CD(26) = Format(IIf((rstC.Tables(0).Rows(0)("Others").ToString()), 0, Val(rstC.Tables(0).Rows(0)("Others").ToString())), "0.00")
            ' CD(39) = Format(IIf((rstC.Tables(0).Rows(0)("AFees").ToString()), 0, Val(rstC.Tables(0).Rows(0)("AFees").ToString())), "0.00")
       ...
       "sum(0) as TotOth FROM Deductee26Q WHERE ChallanID=" & rstC.Tables(0).Rows(c)("ChallanID").ToString()) ', Cnn
            'CD(29) = Format(IIf(String.IsNullOrEmpty(rstDSum.Tables(0).Rows(0)("totdep").ToString()), 0, Val(rstDSum.Tables(0).Rows(0)("totdep").ToString())), "0.00")
            'CD(30) = Format(IIf(String.IsNullOrEmpty(rstDSum.Tables(0).Rows(0)("tottax").ToString()), 0, Val(rstDSum.Tables(0).Rows(0)("tottax").ToString())), "0.00")
            'CD(31) = Format(IIf(String.IsNullOrEmpty(rstDSum.Tables(0).Rows(0)("totsur").ToString()), 0, Val(rstDSum.Tables(0).Rows(0)("totsur").ToString())), "0.00")
            'CD(32) = Format(IIf(String.IsNullOrEmpty(rstDSum.Tables(0).Rows(0)("totecess").ToString()), 0, Val(rstDSum.Tables(0).Rows(0)("totecess").ToString())), "0.00")
            ...
            'CD(34) = Format(IIf(String.IsNullOrEmpty(rstC.Tables(0).Rows(0)("AInterest").ToString()), 0, Val(rstC.Tables(0).Rows(0)("AInterest").ToString())), "0.00")
            'CD(35) = Format(IIf(String.IsNullOrEmpty(rstC.Tables(0).Rows(0)("AOthers").ToString()), 0, Val(rstC.Tables(0).Rows(0)("AOthers").ToString())), "0.00")
            ...
            CD(38) = vbNullString
            'CD(39) already assigned above before challan total
            ...
            For k = 0 To rstChlnDed.Tables(0).Rows.Count - 1
                ...
                DD(6) = "O"
                DD(7) = vbNullString
                DD(8) = IIf(rstChlnDed.Tables(0).Rows(k)("DCode") = "O", 2, 1)
                DD(9) = vbNullString
                DD(10) = rstChlnDed.Tables(0).Rows(k)("DPan").ToString()
                DD(11) = vbNullString
                ' changed to null as per file format ver 4.1
                DD(12) = IIf(rstChlnDed.Tables(0).Rows(k)("DPANCat").ToString() <> 0, rstChlnDed.Tables(0).Rows(k)("Dpanref").ToString(), vbNullString)
                'DD(12) = vbNullString      'FVU ver 3.8 now required refno instead of null
                DD(13) = IIf(String.IsNullOrEmpty(rstChlnDed.Tables(0).Rows(k)("DName").ToString()), "", rstChlnDed.Tables(0).Rows(k)("DName").ToString())
                'DD(14) = Format(IIf(rstChlnDed.Tables(0).Rows(k)("TaxAmt").ToString(), 0, Val(rstChlnDed.Tables(0).Rows(k)("TaxAmt").ToString())), "0.00")
                'DD(15) = Format(IIf((rstChlnDed.Tables(0).Rows(k)("Surcharge").ToString()), 0, Val(rstChlnDed.Tables(0).Rows(k)("Surcharge").ToString())), "0.00")
                'DD(16) = Format(IIf((rstChlnDed.Tables(0).Rows(k)("ECess").ToString()), 0, Val(rstChlnDed.Tables(0).Rows(k)("ECess").ToString())), "0.00")
                ' DD(13) = IIf(String.IsNullOrEmpty(rstChlnDed.Tables(0).Rows(k)("DName").ToString()), "", rstChlnDed.Tables(0).Rows(k)("DName").ToString())
                ...
                DD(20) = vbNullString
                DD(21) = vbNullString
                'DD(22) = Format(IIf((rstChlnDed.Tables(0).Rows(k)("AmtOfPayment").ToString()), 0, Val(rstChlnDed.Tables(0).Rows(k)("AmtOfPayment").ToString())), "0.00")
                ...
                End If
                DD(25) = vbNullString
                If Val(rstChlnDed.Tables(0).Rows(k)("RateOfTDS").ToString()) = 100 Then
                    DD(26) = 99.9999
                Else
                    DD(26) = Format(Val(rstChlnDed.Tables(0).Rows(k)("RateOfTDS").ToString()), "00.0000")
                End If
                ' DD(26) = Format(IIf(rstChlnDed.Tables(0).Rows("RateOfTDS").ToString() = 100, 99.9999, rstChlnDed.Tables(0).Rows(0)("RateOfTDS").ToString()), "00.0000") Then
                DD(27) = vbNullString
                ' DD(28) = CD(37)
                DD(28) = vbNullString       'Changed for FVU 3.8  IIf(rstChlnDed!IsBookEntry = True, "Y", "N")
                DD(29) = vbNullString
                DD(30) = Trim(rstChlnDed.Tables(0).Rows(k)("Remark").ToString())
                DD(31) = vbNullString
                DD(32) = vbNullString
                DD(33) = UCase(Right(rstChlnDed.Tables(0).Rows(k)("Sec").ToString(), 3))
                ...
                ' rstChlnDed.MoveNext
            Next k
            ' If rstChlnDed.State = adStateOpen Then rstChlnDed.Close
            ' rstC.MoveNext
            RecNo = RecNo + 1
        Next c
...
cleanup:
    ...
    Public Sub Convert27ETxt(frmid As String, filename As String, revised As String, oldRRRNo As Double, TANApplNo As Double)
        ...
        Else
            ...
            rstRetn = FetchDataSet("SELECT * FROM RetnMst WHERE CoID=" & selectedcoid & " AND FrmType = 31") ', Cnn
            If rstRetn.Tables(0).Rows.Count <= 0 Then
                Call MsgBox("There is no data for this return form.  Kindly create" _
            & vbCrLf & "this return by using Data entry option and then" _
            & vbCrLf & "create e-TDS file using this option." _
      ...
      "INNER JOIN DeductMst AS DM ON DT.DId = DM.DId WHERE RetnID=" &
        ...
        rstDSum = FetchDataSet("SELECT sum(AmtOfTDS) as TotD FROM Deductee27E WHERE RetnID=" & rstRetn.Tables(0).Rows(0)("RetnID").ToString()) ', Cnn  'Deductee Records
        'START WRITING THE TEXT FILE NOW.
        ...
        ElseIf rstD.Tables(0).Rows.Count = 0 > 999999999 Or rstC.Tables(0).Rows.Count = 0 > 999999999 Then
        ...
        If frmid = "F27E" Then
            ...
            ChallanTotal = (IIf((rstCSum.Tables(0).Rows(0)("TotAmt").ToString()), 0, (rstCSum.Tables(0).Rows(0)("TotAmt").ToString())) +
                    ...
                    IIf((rstCSum.Tables(0).Rows(0)("totecess").ToString()), 0, (rstCSum.Tables(0).Rows(0)("totecess").ToString())) +
                    IIf((rstCSum.Tables(0).Rows(0)("totint").ToString()), 0, (rstCSum.Tables(0).Rows(0)("totint").ToString())) +
                    IIf((rstCSum.Tables(0).Rows("tototh").ToString()), 0, (rstCSum.Tables(0).Rows("tototh").ToString())))
   ...
   Format("Y", "!@@") & Format(oCoMst.PRName26, "!" & New String("@", 75)) &
   ...
   Format(oldRRRNo, New String("0", 14))))   '' revised return number to be incrop...
        ...
        Do While Not rstC.Tables(0).Rows.Count
            CTotal = CLng(IIf((rstC.Tables(0).Rows(0)("amt").ToString()), 0, (rstC.Tables(0).Rows(0)("amt").ToString()))) +
            CLng(IIf((rstC.Tables(0).Rows(0)("Surcharges").ToString()), 0, (rstC.Tables(0).Rows(0)("Surcharges").ToString()))) +
            ...
            CLng(IIf((rstC.Tables(0).Rows(0)("Others").ToString()), 0, (rstC.Tables(0).Rows(0)("Others").ToString()))) * 100
      ...
      Format(IIf((rstC.Tables(0)("BankBrCode").ToString()), Space(7), IIf(rstC.Tables(0).Rows(0)("BankBrCode").ToString() = vbNullString, Space(7), rstC.Tables(0).Rows(0)("BankBrCode").ToString())), "!@@@@@@@") &
      ...
      Format(rstD.Tables(0).Rows(0)("DState"), "00") & Format(rstD.Tables(0).Rows(0)("DPin").ToString(), "000000") &
    ...
    Public Sub Convert2Txt24(filename As String, revised As String, oldRRRNo As Double, TANApplNo As Double)
      ...
      rstRetn.Tables(0).Rows(0)("RetnID").ToString()
        ...
        ChallanTotal = (IIf((rstCSum.Tables(0).Rows(0)("TotAmt").ToString()), 0, (rstCSum.Tables(0).Rows(0)("TotAmt").ToString())) +
                 ...
                 IIf((rstCSum.Tables(0).Rows(0)("totecess").ToString()), 0, (rstCSum.Tables(0).Rows(0)("totecess").ToString())) +
                 IIf((rstCSum.Tables(0).Rows(0)("totint").ToString()), 0, (rstCSum.Tables(0).Rows(0)("totint").ToString())) +
                 IIf((rstCSum.Tables(0).Rows(0)("tototh").ToString()), 0, (rstCSum.Tables(0).Rows(0)("tototh").ToStrings()))
...
Format(IIf(TANApplNo = 0, 0, TANApplNo), New String("0", 14)) & Format(IIf(oldRRRNo = 0, 0, oldRRRNo), New String("0", 14)))
      ...
      Format(CLng(IIf((rstC.Tables(0).Rows(0)("Others").ToString()), 0, (rstC.Tables(0).Rows(0)("Others").ToString()) * 100)), New String("0", 14)) &
      Format(CTotal, New String("0", 14)) &
      Format(IIf((rstC.Tables(0)("ChqDDNo").ToString()), 0, rstC.Tables(0).Rows(0)("ChqDDNo").ToString()), New String("0", 14)) &
      ...
      Format(IIf((rstD.Tables(0).Rows(0)("Remark").ToString()), Space(75), rstD.Tables(0).Rows(0)("Remark").ToString()), "!" & New String("@", 75))
        ...
        Do While Not rstP.Tables(0).Rows.Count
            ...
            For i = 0 To UBound(esr)
                If UCase(Trim(rstP.Tables(0).Rows(0)("DName").ToString())) = UCase(Trim(esr(1))) Then
                    psr = Trim(esr(0))
                    Exit For
                End If
            Next i
            DRec = Format(LNo, "000000000") & "PD" & "000000001" & Format(RecNo, "000000000") &
      Format(rstP.Tables(0).Rows(0)("DName").ToString(), "!" & New String("0", 75)) & Format(psr, "000000000") &
...
cleanup:
    ...
    Public Sub Convert27Txt(frmid As String, filename As String, revised As String)
      ...
      rstRetn.Tables(0).Rows(0)("RetnID").ToString()) ', Cnn   'Deductee Records
        ...
        End If
        If String.IsNullOrEmpty(rstCSum.Tables(0).Rows(0)("totd").ToString()) = True Then
            SumD = 0
        Else
            SumD = CDbl(rstCSum.Tables(0).Rows(0)("totd").ToString() * 100)
        End If
   ...
   Format(SumD, New String("0", 14)) & Format(0, New String("0", 14)) & Space(10) & Format(0, New String("0", 14)))
        ...
        Do While Not rstC.Tables(0).Rows.Count
            TStrm.WriteLine(Format(LNo, "000000000") & "CD" & "000000001" & Format(RecNo, "000000000") & Format(rstC.Tables(0).Rows(0)("Sec").ToString(), "!@@@@@") &
      Format(CLng(rstC.Tables(0).Rows(0)("amt").ToString() * 100), New String("0", 25)) &
      Format(IIf((rstC.Tables(0).Rows(0)("ChallanNo").ToString()), Space(9), IIf(rstC.Tables(0).Rows(0)("ChallanNo").ToString() = vbNullString, Space(9), rstC.Tables(0).Rows(0)("ChallanNo").ToString())), "!@@@@@@@@@") &
      Format((rstC.Tables(0).Rows(0)("DtOfVoucher").ToString()), "ddMMyyyy") &
      Format(IIf((rstC.Tables(0).Rows(0)("BankBrCode").ToString()), Space(7), IIf(rstC.Tables(0).Rows(0)("BankBrCode").ToString() = vbNullString, Space(7), rstC.Tables(0).Rows(0)("BankBrCode").ToString())), "!@@@@@@@"))
            LNo = LNo + 1
