Public Class clsDeductee27EQObj
    'local variable(s) to hold property value(s)
    Private mvarID27EQ As Long 'local copy
    ...
    Private mvarPurchaseVal As Double 'local copy
    ...
    Public Property Sec As String
        ...
        End Set
    End Property
    Public Property PurchaseVal As Double
        Get
            'used when retrieving value of a property, on the right side of an assignment.
            'Syntax: Debug.Print X.ChallanNo
            PurchaseVal = mvarPurchaseVal
        End Get
        Set(ByVal vData As Double)
            'used when assigning a value to the property, on the left side of an assignment.
            'Syntax: X.ChallanNo = 5
            mvarPurchaseVal = vData
        End Set
    End Property
    ...
    Public Function Update(ByVal Deductee As clsDeductee27EQObj) As Boolean
        ...
        End If
        With Deductee
            sql = "Update Deductee27EQ Set RetnId = " & IIf(.RetnID = 0, 0, .RetnID) & "," _
    ...
    Public Function MaxID() As Long
        ...
        Dim sql As String
        sql = "Select Max(Id27EQ) as Id From Deductee27EQ"
    ...
    Public Function Insert(ByVal Deductee As clsDeductee27EQObj) As Boolean
        Dim sql As String, cnl As Boolean
        Dim cmd As New OleDb.OleDbCommand
        Dim transaction As OleDb.OleDbTransaction
        ...
        With Deductee
            ...
            .ID27EQ = MaxID() + 1
            sql = "Insert Into Deductee27EQ (Id27EQ,RetnId,DId,DCode,Sec,AmtOfPayment,DtOfPayment,RateOfTDS,TaxAmt,Surcharge,ECess,TotalTaxDeposited,TotalTaxDeducted,DtOfDeduction,Remark,ChallanID,CertNo,PurchAmt) Values(" _
                & .ID27EQ & ", " & IIf(.RetnID = 0, 0, .RetnID) & ", " _
                & IIf(.did = 0, 0, .did) & ", " _
            ...
            & IIf(.PurchaseVal = 0, 0, .PurchaseVal) & ")"
        End With
...
Err:
    ...
    Public Function Delete(ByVal ID As Long) As Boolean
        ...
        sql = "Delete * From Deductee27EQ Where Id27EQ = " & ID
        cmd.Connection = cn
        transaction = cn.BeginTransaction()
        cmd.Transaction = transaction
    ...
    Public Property ID27EQ As Long
        Get
            'used when retrieving value of a property, on the right side of an assignment.
            'Syntax: Debug.Print X.ID27EQ
            ID27EQ = mvarID27EQ
        End Get
        Set(ByVal vData As Long)
            'used when assigning a value to the property, on the left side of an assignment.
    ...
    Public Function getChallanNo(ByVal ID As Long) As String
        ...
        nds = FetchDataSet("select BankChallanNo,DtOfChallan, TranVouNo from Challan27EQ WHERE challanID=" & ID)
        If nds.Tables(0).Rows.Count > 0 Then
    ...
    Public Function GetChallanID(ByVal BChlNo As Long, ChlDt As Date) As Long
        ...
        nds = FetchDataSet("SELECT challanid FROM Challan27EQ WHERE BankChallanNo=" & BChlNo & " and DtOfChallan=#" & Format(ChlDt, "MM/dd/yyyy") & "#")
        If nds.Tables(0).Rows.Count > 0 Then
